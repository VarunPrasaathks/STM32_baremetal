/* In this code, we'll improvise the previous code UART_command_frames_basic_bm.
 * Over there, we used nested if's after receiving the frame to control the LEDs. that was good for understanding the command structure and working but it was messy and can't be scaled
 * Inorder to maintain the code better, let us use functions and switch case with keeps the code organised and improve readability
 */
