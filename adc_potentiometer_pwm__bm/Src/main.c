/*This project is to execute potentiometer controlled PWM using ADC concepts.
 * A potentiometer is connected to PA0(ADC channel 0) and using that value, we are going to vary PWM at pin PA1(TIM2_Channel 2)
 */
...
int main(){
    ...
    ADC1_SQR3 |=  (0 << 0);
     ...
     while(1){
         while (!(ADC1_SR & (1 << 1)));   // EOC = 1. This bit becomes 1 if ADC conversion has taken place. This condition just exist to wait till ADC conversion.
         //When conversion has taken place, !(1) becomes 0 so we can go on with the code

         uint16_t adc_value = ADC1_DR;    // Read ADC result

         // Map ADC value (0–4095) to PWM duty (0–999)
         uint16_t duty = (adc_value * 1000) / 4095;

         TIM2_CCR2 = duty;                // Update LED brightness
     }
