/*This code aims to experiment an important aspect of data framing - the checksum
 * till now, our FSM assumed that whatever the data arrived was valid, provided it was valid according to the logic in the code
 * but in real life scenarios, this may not be the case, bytes may get corrupted/changed due to various reasons and we have to ensure that this problem is under check
 * for this purpose, we introduce checksum into the FSM which performs certain operations with the original and received data to check for integrity
 * here let us perform XOR for the sent and received frames separately. if the XOR value remains the same, our data frame is correct.
 */
...
void rx_fsm(uint8_t byte){
	...
	case RX_WAIT_CHK:
		if(byte == rx.checksum){
			...
			GPIOA_ODR |= (1U << 3);  //PA3 glows if the correct XOR of the payload is entered . for example 0x11 ^ 0x22 ^ 0x33 = 0. if 0 is sent, PA3 will glow
		}else{
			rx.state = RX_WAIT_SOF;
		}
		break;
	...
	case RX_WAIT_EOF:
		if(byte == 0x55){
		rx.state = RX_WAIT_SOF;
		GPIOA_ODR |= (1U << 4);  //PA4 will glow only when the correct EOF byte is entered. all the other bytes after payload is neglected if it is not 0x55.
	}
		break;
	}
}
